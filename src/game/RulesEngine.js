import { SAFE_ABS, HOME_COL, HOME_START, FINISHED, PATH_LEN } from '../constants/Board';
export class RulesEngine {
    constructor(players) {
        this.players = players;
    }
    // Retorna se uma peça pode mover dado um valor de dado
    canMove(piece, player, dice) {
        if (piece.piecePos === FINISHED)
            return false;
    ...
    calculateMove(piece, player, dice) {
        if (!this.canMove(piece, player, dice))
            return null;
        const pathSteps = [];
        let newPos;
        if (piece.piecePos === -1) {
            ...
            if (newHomeIdx === HOME_COL[player.color].length) {
                ...
                for (let i = homeIdx + 1; i <= newHomeIdx; i++)
                    pathSteps.push(HOME_START + i);
            }
        }
        else {
            // Anel externo
            ...
            if (dice >= distToEntry) {
                ...
                for (let s = relPos + 1; s < PATH_LEN; s++)
                    pathSteps.push(s);
                const homeSteps = dice - distToEntry;
                if (homeSteps === 0) {
                    ...
                    newPos = FINISHED;
                    for (let h = 0; h < homeSteps; h++)
                        pathSteps.push(HOME_START + h);
    ...
    findCaptures(movingPiece, movingPlayer, newPos) {
        // Sem captura dentro da coluna do lar ou quando termina
        if (newPos >= HOME_START || newPos === FINISHED || newPos === -1)
            return [];
