...
import { PLAYER_COLORS, C } from '../constants/Colors';
import { BOARD_PX, OX } from '../constants/Board';
...
export class GameScene extends Phaser.Scene {
    ...
    async onDiceClicked() {
        if (this.animating || this.waiting)
            return;
        const player = this.players[this.curIdx];
        if (player.isAI)
            return;
    ...
    async onPieceClicked(piece) {
        if (!this.waiting || this.animating)
            return;
